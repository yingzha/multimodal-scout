...
def scrape_huggingface_trending_papers() -> List[SourceSchema]:
    ...
    try:
        ...
        if not papers_json_list:
            logger.warning("Could not find 'dailyPapers' data. The website structure may have changed.")
            return []
        ...
        for paper_entry in papers_json_list:
            ...
            if not all([paper_id, title, published_at]):
                logger.warning(f"Skipping paper due to missing essential data (id, title, or date): {title or 'N/A'}")
                continue
        ...
        logger.error(f"An unexpected error occurred during Hugging Face scrape: {err}", exc_info=True)

    logger.info(f"Successfully scraped and validated {len(validated_papers)} papers from Hugging Face.")
